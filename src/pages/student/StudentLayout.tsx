...
import {
  LayoutDashboard, Calendar, IndianRupee, BookOpen, Bell,
  Settings, LogOut, GraduationCap, Menu, X, PlusCircle,
  Video, TrendingUp, Sun, Moon, ChevronDown,
} from 'lucide-react';
...
export default function StudentLayout() {
  ...
  useEffect(() => {
    async function checkAuth() {
      ...
      const name = (profile?.full_name as string) || user.user_metadata?.full_name as string || 'Student';
      setProfileName(name);
    }
    checkAuth();
  }, [navigate]);
  ...
  const navItems = [
    ...
    { name: 'Fees', href: '/student/fees', icon: IndianRupee },
    ...
    { name: 'Join Batch', href: '/student/join', icon: PlusCircle },
  ];
    ...
    <div className="min-h-screen flex" style={{ backgroundColor: 'var(--background)' }}>
      ...
      <aside className={cn(
        ...
        <div className={cn('px-5 h-16 flex items-center gap-2.5 font-bold text-xl tracking-tight border-b shrink-0', sidebarBorder)}>
          <div className="bg-gradient-to-br from-violet-500 to-purple-600 text-white p-1.5 rounded-xl shadow-md">
        ...
        <div className="px-4 py-3">
          <div className="flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-bold uppercase tracking-widest text-violet-600 bg-violet-50 dark:bg-violet-950 border border-violet-200 dark:border-violet-900">
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  active
                    ? 'bg-gradient-to-r from-violet-500 to-purple-600 text-white shadow-md shadow-violet-200 dark:shadow-violet-900'
        ...
        <header className={cn(
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="ghost" size="icon" className="relative h-9 w-9">
              <Bell size={18} />
              <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-violet-500 rounded-full border border-white dark:border-slate-900" />
            </Button>
              ...
              {userMenuOpen && (
                <div className="absolute right-0 top-full mt-2 w-48 rounded-2xl border shadow-xl overflow-hidden z-50"
                  ...
                  <div className="px-4 py-3 border-b" style={{ borderColor: 'var(--card-border)' }}>
                    ...
                    <p className="text-xs text-violet-500 font-semibold">Student</p>
                  </div>
