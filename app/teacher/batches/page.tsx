...
export default function BatchesPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Batch Grid */}
      {loading ? (
        <div className="text-center py-20 text-muted-foreground italic font-serif">Loading your batches...</div>
      ) : filteredBatches.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredBatches.map((batch) => (
            <Card key={batch.id} className="group hover:border-primary transition-colors overflow-hidden">
              <CardHeader className="pb-4">
                <div className="flex items-start justify-between">
                  <div className="space-y-1">
                    <CardTitle className="text-xl font-bold">{batch.name}</CardTitle>
                    <p className="text-sm text-muted-foreground font-medium uppercase tracking-wide">{batch.subject}</p>
                  </div>
              ...
              <CardContent className="space-y-4 pb-4">
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div className="flex items-center gap-2 text-muted-foreground">
                    <Users size={16} />
                    <span>{batch.enrollments[0]?.count || 0} / {batch.max_students} Students</span>
                  </div>
                  <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <div className="p-3 bg-secondary/30 rounded-lg flex items-center justify-between">
                  ...
                  <Button 
                    variant="ghost" 
                    size="icon" 
                    onClick={() => copyInviteCode(batch.invite_code)}
              ...
              <CardFooter className="pt-0">
                <Link href={`/teacher/batches/${batch.id}`} className="w-full">
